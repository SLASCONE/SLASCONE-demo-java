  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SlasconeLicenseInfoDto.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException("The required field(s) %s in SlasconeLicenseInfoDto is not found in the empty JSON string".formatted(SlasconeLicenseInfoDto.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SlasconeLicenseInfoDto.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException("The field `%s` in the JSON string is not defined in the `SlasconeLicenseInfoDto` properties. JSON: %s".formatted(entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("error_message") != null && !jsonObj.get("error_message").isJsonNull()) && !jsonObj.get("error_message").isJsonPrimitive()) {
        throw new IllegalArgumentException("Expected the field `error_message` to be a primitive type in the JSON string but got `%s`".formatted(jsonObj.get("error_message").toString()));
      }
